...
const ImageBase64 = (props: any) => {
  ...
  useEffect(() => {
    ...
    t.onChannel('report-test-image-response', (data: ExtendSocketData) => {
      const _data = data?.data as any;
      const apiRes = _data?.testResult;
      const _requestId = _data?.requestId;
...
export const TestAiApi: FC = () => {
  ...
  useEffect(() => {
    const t = TimsSocket.init();
    t.onChannel('report-test-image-request', (data: ExtendSocketData) => {
